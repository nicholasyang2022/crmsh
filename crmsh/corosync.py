...
def get_local_node_ring_addresses() -> list[str]:
    """
...
def get_corosync_interfaces(interfaces_inst: typing.Optional[network_utils.InterfacesInfo] = None) -> list[str]:
    """
    Get unique NIC names configured for Corosync on this node.
    """
    ring_addrs = get_local_node_ring_addresses()
    if not ring_addrs:
        return []

    if interfaces_inst is None:
        interfaces_inst = network_utils.InterfacesInfo()

    corosync_nics = []
    for addr in ring_addrs:
        nic = None
        try:
            nic = interfaces_inst.get_nic_by_subnet_of_addr(addr)
        except Exception:
            pass
        if not nic:
            try:
                nic = interfaces_inst.get_nic_name_by_addr(addr)
            except Exception:
                pass
